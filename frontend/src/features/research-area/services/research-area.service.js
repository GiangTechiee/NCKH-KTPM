...
function mapResearchArea(area) {
  return {
    ...
    groupCount: area.soLuongNhomHienTai || 0,
    groupLimit: area.gioiHanSoNhom || 5,
...
async function layNhomTheoMang(researchAreaId) {
  const response = await apiClient.getJson(`/api/mang-nghien-cuu/${researchAreaId}/nhom`);
  return response?.data || {
    mangNghienCuuId: researchAreaId,
    soLuongNhomHienTai: 0,
    gioiHanSoNhom: 5,
    coTheTaoNhomMoi: true,
    nhomHienCo: [],
  };
}
