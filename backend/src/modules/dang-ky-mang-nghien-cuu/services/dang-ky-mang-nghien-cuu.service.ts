...
class DangKyMangNghienCuuService {
  ...
  async layDanhSachMangDangMo(): Promise<MangNghienCuuDangMoResponse[]> {
    ...
    return danhSach.map((mang) => ({
      ...
      soLuongNhomHienTai: mang._count.nhomNghienCuu,
      gioiHanSoNhom: MAX_GROUPS_PER_AREA,
    }));
  }

  async layLuaChonNhomTheoMang(mangNghienCuuId: bigint) {
  const mang = await this.dangKyMangNghienCuuRepository.timMangTheoId(mangNghienCuuId);
  if (!mang) {
    throw new NotFoundError('Không tìm thấy mảng nghiên cứu', [
      { field: 'mangNghienCuuId', code: 'RESEARCH_AREA_NOT_FOUND' },
    ]);
  }

  const nhom = await this.dangKyMangNghienCuuRepository.timDanhSachNhomTheoMang(mangNghienCuuId);

  return {
    mangNghienCuuId,
    soLuongNhomHienTai: nhom.length,
    gioiHanSoNhom: MAX_GROUPS_PER_AREA,
    coTheTaoNhomMoi: nhom.length < MAX_GROUPS_PER_AREA,
    nhomHienCo: nhom.map((group) => {
      const truongNhom = group.thanhVien.find((item) => item.vaiTro === 'TRUONG_NHOM');

      return {
        id: group.id,
        tenNhom: group.tenNhom,
        soLuongThanhVien: group.soLuongThanhVien,
        coTheThamGia: group.soLuongThanhVien < MAX_GROUP_MEMBERS,
        truongNhom: truongNhom
          ? {
              maSinhVien: truongNhom.sinhVien.maSinhVien,
              hoTen: truongNhom.sinhVien.hoTen,
            }
          : null,
      };
    }),
  ...
  async dangKyMang(sinhVienId: bigint, input: DangKyMangNghienCuuDto): Promise<DangKyMangResponse | ChuyenMangResponse> {
    ...
    const danhSachNhom = await this.dangKyMangNghienCuuRepository.timDanhSachNhomTheoMang(input.mangNghienCuuId);
    const daHetChoHoanToan =
      danhSachNhom.length >= MAX_GROUPS_PER_AREA
      && danhSachNhom.every((group) => group.soLuongThanhVien >= MAX_GROUP_MEMBERS);

    if (daHetChoHoanToan) {
      throw new ConflictError({
        message: 'Mảng nghiên cứu này đã đủ 5 nhóm và tất cả nhóm đều đủ thành viên',
        errorCode: 'RESEARCH_AREA_GROUP_CAPACITY_REACHED',
        errors: [{ field: 'mangNghienCuuId', code: 'AREA_FULLY_GROUPED' }],
      });
    }
    if (!mangNghienCuu) {
