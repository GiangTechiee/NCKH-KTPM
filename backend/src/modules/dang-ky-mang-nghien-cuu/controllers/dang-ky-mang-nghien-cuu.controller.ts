...
import { xacThucDangKyMangNghienCuu, layMangIdTuRequest } from '../validators/dang-ky-mang-nghien-cuu.validator';
...
class DangKyMangNghienCuuController {
  ...
  async layLuaChonNhomTheoMang(request: Request, response: Response): Promise<Response> {
  const mangId = layMangIdTuRequest(request);
  const data = await dangKyMangNghienCuuService.layLuaChonNhomTheoMang(mangId);

  return sendSuccess(response, {
    message: 'Lấy danh sách nhóm theo mảng nghiên cứu thành công',
    data,
  });
}
