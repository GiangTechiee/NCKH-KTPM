...
class NhomNghienCuuService {
  ...
  async taoNhomNghienCuu(sinhVienId: bigint, input: TaoNhomNghienCuuDto) {
    ...
    const soNhomHienTai = await this.nhomNghienCuuRepository.demSoNhomTheoMang(dangKyMang.mangNghienCuuId);
      if (soNhomHienTai >= MAX_GROUPS_PER_AREA) {
        throw new ConflictError({
          message: 'Mảng nghiên cứu này đã đạt giới hạn 5 nhóm',
          errorCode: 'AREA_GROUP_LIMIT_REACHED',
        });
      }
