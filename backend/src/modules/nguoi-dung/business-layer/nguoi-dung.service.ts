...
import { NguoiDungRepository } from '../data-access-layer/nguoi-dung.repository';
...
class NguoiDungService {
  ...
  async laySinhVienTheoMa(maSinhVien: string): Promise<SinhVien> {
  ...
  async layGiangVienTheoMa(maGiangVien: string): Promise<GiangVien> {
  ...
  async lietKeTaiKhoanGiangVien(): Promise<LecturerAccountOption[]> {
    ...
    return danhSachGiangVien.map((giangVien) => ({
      ...
      recentGroups: giangVien.nhomNghienCuu.map((nhom) => ({
